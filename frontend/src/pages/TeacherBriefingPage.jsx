...
const TeacherBriefingPage = () => {
  ...
  useEffect(() => {
    const fetchBriefing = async () => {
      if (!token || !classId) {
        setIsLoading(false);
        setError('Teacher briefing requires an authenticated teacher session.');
        return;
      }
      try {
        setIsLoading(true);
        setError('');
        const response = await fetch(`${apiUrl}/teachers/classes/${classId}/briefing/export`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        if (!response.ok) {
          const detail = await response.json().catch(() => null);
          throw new Error(detail?.detail || 'Failed to load teacher briefing.');
        }
        const data = await response.json();
        setBriefing(data || null);
      } catch (err) {
        setBriefing(null);
        setError(err.message || 'Teacher briefing is unavailable right now.');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleDownload = () => {
    ...
    anchor.href = url;
    anchor.download = briefing.file_name || 'class-briefing.md';
    anchor.click();
      ...
      <div className="mx-auto max-w-5xl space-y-6">
        <div className="flex flex-col gap-4 rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm print:hidden">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <h1 className="text-2xl font-black text-slate-900">Teacher briefing</h1>
              <p className="mt-1 text-sm text-slate-500">
                Print-friendly graph-backed class summary for lesson planning and presentation.
              </p>
            </div>
        ...
        {isLoading ? (
          <div className="flex min-h-[320px] flex-col items-center justify-center rounded-[28px] border border-slate-200 bg-white text-slate-400 shadow-sm">
            ...
            <p className="text-sm font-semibold">Preparing briefing...</p>
          </div>
        ) : error ? (
          ...
          <div className="rounded-[28px] border border-slate-200 bg-white p-6 text-sm font-semibold text-slate-500 shadow-sm">
            No briefing is available for this class yet.
          </div>
        ) : (
          <div className="space-y-6">
            <section className="rounded-[28px] border border-slate-200 bg-white p-8 shadow-sm print:shadow-none">
              <p className="text-[11px] font-black uppercase tracking-[0.2em] text-indigo-500">Class briefing</p>
              ...
              <div className="mt-6 grid gap-4 md:grid-cols-3">
                <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                  <p className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-400">Class</p>
                  <p className="mt-2 text-lg font-black text-slate-900">{briefing.class_name}</p>
                </div>
                <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                  <p className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-400">Scope</p>
                  <p className="mt-2 text-lg font-black text-slate-900">
                    {briefing.subject.toUpperCase()} {briefing.sss_level} Term {briefing.term}
                  </p>
                </div>
                <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
...
export default TeacherBriefingPage;
